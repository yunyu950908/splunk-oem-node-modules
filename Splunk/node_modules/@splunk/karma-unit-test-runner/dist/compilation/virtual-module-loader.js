...
//if no merging utility is available or for non-core build environments
loader.install = function (config, wpVersion) {
    _.set(config, 'resolve.alias.virtual-module-template', loader.config.resolve.alias['virtual-module-template']);

    var resolveConfigPath = 'resolveLoader.' + (wpVersion >= 2 ? 'modules' : 'root');
    var loaderResolve = _.get(config, resolveConfigPath, wpVersion >= 2 ? ['node_modules'] : []);
    loaderResolve.push(__dirname);
    _.set(config, resolveConfigPath, loaderResolve);
};

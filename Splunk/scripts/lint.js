...
var config = {
    js: {
        postAce: {
            patterns: ['search_mrsparkle/exposed/js/*/'], // the /*/ at the end is intentional: include only subdirs
            options: [],
            reportRoot: path.join('search_mrsparkle', 'exposed', 'js')
        },
        preBubbles: {
            patterns: ['search_mrsparkle/exposed/js/*.js'], // exclude all subdirs
            options: ['-c', path.join('.', '.jshintrc_pre_bubbles')],
            reportRoot: path.join('search_mrsparkle', 'exposed', 'js')
        },
    ...
    es: {
        es: {
            patterns: ['.',
                '../cfg/bundles/corejs_test',
                '../cfg/bundles/splunkjs_test'],
            options: ['--ext', '.es,.jsx'],
            reportRoot: '' //currently ignored
        }
    }
};
...
var xmlOutputDir = path.join('.', 'ci_ui_xml');

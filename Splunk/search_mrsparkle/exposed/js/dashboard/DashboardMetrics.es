...
import { isValidPivotSearch } from 'util/general_utils';
...
export function captureDashboardMetrics(model, state, managers) {
    ...
    const elementTypeCounts = getElementTypeCounts(state.elements, model.reportDefaults);
    ...
    const searchCountsPromise = getSearchTypeCounts(state.searches, searchManagers);
 ...
 */
export function getElementTypeCounts(elements, reportDefaults) {
 ...
 */
export function getSearchTypeCounts(searches, managers) {

...
define([
    ...
    var BaseModule = _.extend({}, qunitUtils.FakeXhrModule, {
        ...
        setup: function() {
            ...
            this.WINDOWED_EARLIEST = '1981-08-18T00:00:00.000-07:00';
            this.WINDOWED_LATEST = '1984-02-17T00:00:00.000-07:00';
            ...
            this.verifyAndRespondToCollect = function(expectedDispatchAttrs, customResponseAttrs, options) {
                ...
                if (options.useTimeWindow) {
                    expectedDispatchAttrs.earliest_time = this.WINDOWED_EARLIEST;
                    expectedDispatchAttrs.latest_time = this.WINDOWED_LATEST;
                }
            ...
            this.verifyUrlParams = function() {
                ...
                if (_(attrsToCheck).contains('windowedEarliest')) {
                    assert.equal(urlAttrs.windowedEarliest, this.WINDOWED_EARLIEST, 'the "windowedEarliest" URL param is correct');
                    attrsToCheck = _(attrsToCheck).without('windowedEarliest');
                }
                if (_(attrsToCheck).contains('windowedLatest')) {
                    assert.equal(urlAttrs.windowedLatest, this.WINDOWED_LATEST, 'the "windowedLatest" URL param is correct');
                    attrsToCheck = _(attrsToCheck).without('windowedLatest');
                }
            ...
            this.verifyFinalReportState = function(timeRange, options) {
                ...
                var expectedPivotJobParams = {
                    ...
                    earliest_time: options.useTimeWindow ? this.WINDOWED_EARLIEST : earliest,
                    app: this.application.get('app'),
                    owner: this.application.get('owner'),
                    provenance: 'UI:Pivot'
                };
                if(latest || options.useTimeWindow) {
                    expectedPivotJobParams.latest_time = options.useTimeWindow ? this.WINDOWED_LATEST : latest;
                }
            ...
            this.respondCoreVisualizations = function() {
                this.respondToMatchingRequest(
                    /data\/ui\/visualizations/,
                    MockCoreVisualizations.getCoreVisualizationsSplunkdResponse()
                );
            };
            ...
            this.quickSetupNewReport = function(uriParams) {
                ...
                this.respondCoreVisualizations();

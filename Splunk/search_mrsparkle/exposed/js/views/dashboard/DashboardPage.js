define([
    ...
    return BaseView.extend({
        ...
        render: function() {
            ...
            this.renderTour();
            // DCE-OEM-CHANGE 没有 appbar 时候, 不需要 margin-left
            var mainSection = this.$el.children('.main-section-body');
            var left = mainSection.css('margin-left');
            if ((this.model.renderState.get('hideChrome') || this.model.renderState.get('hideAppBar')) && left === '200px') {
                mainSection.css('margin-left', '0px');
            }
            if (!this.model.renderState.get('hideChrome') && !this.model.renderState.get('hideAppBar') && left === '0px') {

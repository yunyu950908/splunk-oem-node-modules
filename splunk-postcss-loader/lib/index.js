...
var AUTOPREFIXER_BROWSERS = ['Android 2.3', 'Android >= 4', 'Chrome >= 35', 'Firefox >= 31', 'Explorer >= 11', 'Edge >= 1', 'iOS >= 7', 'Opera >= 12', 'Safari >= 7.1'];
...
function themeLoader(source) {
    ...
    function generateScopedName(name, filename) {
        hashes.filename = hashes.filename || crypto.createHash('md5').update(filename).digest('hex').substr(0, 6);
    ...
    function loadtheme(theme) {
        ...
        postcss(postcssPlugins).process(source, postcssOptions).then(function (result) {
            ...
            return result;
        }).then(receiveCSS).catch(catchError);
    }

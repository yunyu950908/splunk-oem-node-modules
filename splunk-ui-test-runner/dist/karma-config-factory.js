...
module.exports = function buildConfig(config, settings) {
    ...
    config.set({
        ...
        frameworks: ['webpack', 'mocha', 'chai'].concat(settings.features.sinon ? 'sinon' : []),
    ...
    if (config.saucelabs) {
        ...
        config.set({
            ...
            sauceLabs: {
                testName: 'splunk-ui-test-runner'
            },
    ...
    if (config.coverage || false) {
        ...
        config.coverageReporter = {
            ...
            reporters: [{ type: 'in-memory' }, // emits coverage_complete when done
            { type: 'html' }]
        };
    ...
    Object.keys(testApps).forEach(function (app, idx, apps) {
        ...
        fileFilter.forEach(function (filter) {
            ...
            filterPatterns.forEach(function (f) {
                config.files.push({
                    pattern: path.join(appTestPath, f),
                    included: false
                });
            });
        });

...
function renderDays({ selectedDate, startDate, endDate, onChange }) {
...
function renderHeader() {
    return (
        <thead>
            <tr>
                {times(7, (i) => {
                    const label = moment().weekday(i).format('dd');
                    return (
...
export default function DateTable(props) {
    const { displayValue, onChange, selectedValue } = props;
    ...
    const endDate = moment(displayDate).endOf('month').startOf('dat');

    const cells = renderPaddingCells(startDate.format('e'))
        .concat(renderDays({ selectedDate, startDate, endDate, onChange }))

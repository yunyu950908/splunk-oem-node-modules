...
var karmaRuns = [
    ...
    { name: 'splunk_instrumentation', args: ['--apps', 'splunk_instrumentation'] }
];
...
var karmaBaseArgs = [karmaPath, 'start', '--single-run', '--browsers', 'PhantomJS',
...
function buildCommands() {
    ...
    var karmaCmds = karmaRuns.map(function (run) {
        return karmaBaseArgs.concat(
            '--junit-directory', path.join('web', xmlOutputDir), '--junit-filename', 'test_' + run.name + '.xml',
            run.args).join(' ');
    }).join('\n');
...
function runParallel() {
    ...
    cproc.spawnSync(parallelPath, ['--dry-run'].concat(parallelKarmaArgs), { cwd: karmaCwd, stdio: 'inherit' });
    ...
    console.log('');
    Promise.all([
        spawn('karma', parallelPath, parallelKarmaArgs, karmaCwd, karmaExitCodeCallback),
...
function karmaExitCodeCallback(code) {
    ...
    return karmaRuns.some(function (run) {
        ...
        var outputFile = path.join(xmlOutputDir, 'test_' + run.name + '.xml');
...
//prepare output dir and clean any lingering intermediary files

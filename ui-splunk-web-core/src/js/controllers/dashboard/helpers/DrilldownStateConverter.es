...
const hasAppName = link => /^\/app\/\w+\/\w+/.test(link);
const getAppName = (link) => {
    const matches = link.match(/^\/app\/(\w+)\/\w+/);
    if (matches) {
        return matches[1];
    }
...
const isReportLink = link => /^(\/app\/\w+\/)?report\?s=.*/.test(link);
...
const getReportName = (link) => {
    if (hasAppName(link)) {
        return link.match(/^\/app\/\w+\/report\?s=(.+)/)[1];
    }

    return link.slice(9); // example: 'report?s=report_name'
};

const getDashboardNameAndParams = (link) => {
    if (hasAppName(link)) {
        return link.match(/^\/app\/\w+\/(\w+.*)/)[1];
    }
            ...
            Object.assign(drilldown, {
                settings: {
                    actions: [{
                        target: setting.target,
                        type: 'link',
                        value: `/app/${setting.activeApp}/report?s=${setting.activeReport}`,
                    }],
                },
            });
            break;

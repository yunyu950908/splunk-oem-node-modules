...
define([
    ...
    var PivotReport = Report.extend({
        ...
        getNumFilters: function(includeIndexTime) {
            includeIndexTime = includeIndexTime !== false;

            if (includeIndexTime) {
                return this.getElementCollectionByType('filter').length;
            }

            return this.getElementCollectionByType('filter').filter(function (filter) {
        ...
        setFromPivotJSON: function(pivotJSON, options) {
            ...
            // Guard against inconsistent input from the pivot search parsing endpoint.  In scenarios where split limit amounts are disabled,
            // the endpoint will encode a value of 0.  This should be normalized to the default value when setting on this model.
            if (!!this.validateLimitAmountEnabled('row')) {
                setObject.rowLimitAmount = CONTENT_DEFAULTS.rowLimitAmount;
            }
            if (!!this.validateLimitAmountEnabled('column')) {
                setObject.colLimitAmount = CONTENT_DEFAULTS.colLimitAmount;
            }
        ...
        getPivotJSON: function() {
            ...
            if(columnJSON.length > 0) {
                ...
                if(!this.validateLimitAmountEnabled('column')) {
                    ...
                    pivotJSON.colFormat.limitAmount = MAX_COLUMN_LIMIT;
                }
            }

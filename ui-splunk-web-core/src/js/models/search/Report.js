define(
    ...
    function($, _, SavedSearch, Viewstate, Cron, JobsCollection, HistoriesCollection, time_utils, splunkd_utils, general_utils, splunkUtil, i18n, route, math_utils) {
        /**
         * @constructor
         * @memberOf models
         * @name Report
        ...
        var ReportModel = SavedSearch.extend({
            ...
            getStringOfActions: function() {
                ...
                if (this.entry.content.get("action.email") || this.entry.content.get("action.script") || this.entry.content.get("action.lookup")) {
                    ...
                    if (actions.search('lookup') != -1) {
                        actionArray.push(_('Write to a Lookup').t());
                    }
                }
            ...
            getScheduleAndActionsString: function() {
                ...
                if (!this.isRealTime()) {
                    if (this.entry.content.get('is_scheduled')) {
                        ...
                        actionsString = this.getStringOfActions();

                        if (actionsString.length) {
                            text += splunkUtil.sprintf(_(' Actions: %s.').t(), actionsString);
                        } else {
                            text += _(' No actions.').t();
                        }
                    } else {

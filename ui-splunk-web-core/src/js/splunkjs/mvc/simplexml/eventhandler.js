define([
    ...
    var EventHandler = BaseManager.extend({
        ...
        executeAction: function (action, data, evt) {
            ...
            switch (action.type) {
                ...
                case "link":
                    ...
                    var application = this._component.model.application;
                    return EventHandler.linkTo(action.value, action.target || target, data, newWindow, application);
                case "eval":
        ...
        linkTo: function (urlFormat, target, data, newWindow, application) {
            if (urlFormat[0] === '/' && urlFormat[1] !== '/') {
                urlFormat = SplunkUtil.make_url(urlFormat);
            } else if (urlFormat[0] !== '/' && application) {
                // SPL-138171: always make full url to make sure link works in edit mode.
                urlFormat = SplunkUtil.make_url('/app/' + encodeURIComponent(application.get('app')) + '/' + urlFormat);
            }

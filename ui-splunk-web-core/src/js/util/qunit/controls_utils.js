...
define(['jquery',
    ...
    var BaseControlModule = {
        ...
        readControlValue: function(controlView) {
            ...
            if (controlView instanceof SyntheticCheckboxControl) {
                return this.readSyntheticCheckboxValue(controlView.$el);
            }
        ...
        readSyntheticCheckboxValue: function($el) {
            return $el.find('.icon-check').is(':visible');
        },
        ...
        writeControlValue: function(controlView, value) {
            ...
            else if(controlView instanceof SyntheticCheckboxControl) {
                this.toggleSyntheticCheckbox(controlView.$el);
            }
            else {
                throw new Error('Unsupported control type');
            }
        ...
        toggleSyntheticCheckbox: function($el) {
        ...
        verifyControlEnabled: function(controlView, model, modelAttribute) {
            ...
            if(controlView instanceof TextControl || controlView instanceof TextareaControl) {
                ...
                assert.ok(true, 'initial checkbox value is 0');
                assert.equal(this.readSyntheticCheckboxValue(controlView.$el), 0);
                assert.ok(true, 'toggle the checkbox');
                this.writeControlValue(controlView);
                assert.equal(model.get(modelAttribute), 1, 'the model was updated correctly');
                assert.ok(true, 'setting the model attribute to 0');
                model.set(modelAttribute, 0);
                assert.equal(this.readControlValue(controlView), 0, 'the checkbox was updated correctly');
            }
            else {
                throw new Error('Unsupported control type');
            }

    ...
    var JSChart = VisualizationBase.extend({
        ...
        formatData: function(rawResults) {
            if (!rawResults || !rawResults.columns || rawResults.columns.length === 0) {
                return jschartingUtils.extractChartReadyData({fields:[], columns:[]});
            }
            ...
            var resultsDataSet = jschartingUtils.extractChartReadyData(results);
        ...
        provideScaleValues: function(dataSet, config){
            if(dataSet.seriesList && dataSet.seriesList.length > 1) {
                var fields = fieldUtils.getChartFieldData(dataSet);

                var yAxisFields = fields.yFields;

                // If in overlay mode, we split the scale
                if (parsingUtils.normalizeBoolean(config['display.visualizations.charting.axisY2.enabled'])){
        ...
        _updateViewAfterLoad: function(dataSet, config, async) {
            var done = async();
            ...
            if (!_.isUndefined(this.getScale('yAxis'))) {
                var yAxisExtremes = { 
                    yAxisMin: this.getScale('yAxis').get('actualMinimum'),
                    yAxisMax: this.getScale('yAxis').get('actualMaximum')
                };
            }
            if (!_.isUndefined(this.getScale('overlayAxis'))) {
                var overlayAxisExtremes = { 
                    overlayAxisMin: this.getScale('overlayAxis').get('actualMinimum'),
                    overlayAxisMax: this.getScale('overlayAxis').get('actualMaximum')
                };
            }
